...
def test_obtain_deployed_bridge_addresses():
    ...
    print(arb_one_l2_network)
    eth_provider = Web3(Web3.HTTPProvider(os.getenv("http://127.0.0.1:7545")))

    # Obtain on-chain information
    eth_bridge = get_eth_bridge_information(arb_one_l2_network.eth_bridge.rollup, eth_provider)

    # Assertions
    assert arb_one_l2_network.eth_bridge.rollup == eth_bridge.bridge, "Bridge contract is not correct"
