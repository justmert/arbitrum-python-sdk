...
class InboxTools:
    def __init__(self, l1_signer: Any, l2_network: Any):
        ...
        if not self.l1_network:
            raise ArbSdkError(
                f"L1Network not found for chain id: {l2_network.partner_chain_id}."
            )

    async def find_first_block_below(
        self, block_number: int, block_timestamp: int
    ) -> Any:
        ...
        return await self.find_first_block_below(
            block_number - diff_blocks, block_timestamp
        )
    ...
    async def estimate_arbitrum_gas(
        self, transaction_l2_request, l2_provider
    ) -> Dict[str, Any]:
        ...
        gas_components = node_interface.functions.gasEstimateComponents(
            transaction_l2_request["to"]
            or Web3.to_checksum_address("0x0000000000000000000000000000000000000000"),
            contract_creation,
        ...
        gas_components = format_contract_output(
            node_interface, "gasEstimateComponents", gas_components
        )

        gas_estimate_for_l2 = (
            gas_components["gasEstimate"] - gas_components["gasEstimateForL1"]
        )
        return {**gas_components, "gasEstimateForL2": gas_estimate_for_l2}

    async def get_force_includable_block_range(
        self, block_number_range_size: int
    ) -> Dict[str, int]:
        ...
        multicall_input: Tuple[Any, Any, Any] = [
            {
                "targetAddr": sequencer_inbox.address,
                "encoder": lambda: sequencer_inbox.encode_function_data(
                    "maxTimeVariation"
                ),
                "decoder": lambda return_data: sequencer_inbox.decode_function_result(
                    "maxTimeVariation", return_data
                )[0],
            },
        ...
        first_eligible_block_number = (
            current_block_number - max_time_variation.delay_blocks
        )
        first_eligible_timestamp = (
            current_block_timestamp - max_time_variation.delay_seconds
        )

        first_eligible_block = await self.find_first_block_below(
            first_eligible_block_number, first_eligible_timestamp
        )
    ...
    async def get_events_and_increase_range(
        ...
        block_range = await self.get_force_includable_block_range(
            capped_search_range_blocks
        )
    ...
    async def sign_l2_tx(self, tx_request, l2_signer: SignerProviderUtils) -> str:
        ...
        if not is_defined(tx.get("nonce", None)):
            tx["nonce"] = l2_signer.provider.eth.get_transaction_count(
        ...
        if tx.get("type") == 1 or "gasPrice" in tx:
            ...
            if not is_defined(tx.get("maxFeePerGas", None)):
                fee_data = l2_signer.provider.eth.fee_history(
                    1, "latest", reward_percentiles=[]
                )
        ...
        if not is_defined(tx.get("to", None)):
            tx["to"] = Web3.to_checksum_address(
                "0x0000000000000000000000000000000000000000"
            )

        try:
            tx["gas"] = (await self.estimate_arbitrum_gas(tx, l2_signer.provider))[
                "gasEstimateForL2"
            ]
