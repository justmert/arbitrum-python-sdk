...
class L1ToL2Message:
    ...
    def calculate_submit_retryable_id(
        ...
        dest_addr = (
            "0x"
            if dest_address == ADDRESS_ZERO
            else Web3.to_checksum_address(dest_address)
        )
        ...
        if creation_receipt:
            ...
            if len(redeem_events) == 1:
                return self.l2_provider.eth.get_transaction_receipt(
                    self.l2_provider, redeem_events[0]["retryTxHash"]
                )
        ...
        while from_block.number < max_block:
            ...
            successful_redeem = list(
                filter(
                    ...
                    [
                        await self.l2_provider.get_transaction_receipt(
                            e.event["retryTxHash"]
                        )
...
class L1ToL2MessageWriter(L1ToL2MessageReader):
    ...
    async def redeem(self, overrides=None):
        ...
        if status == L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2:
            ...
            tx_receipt = self.l2_signer.provider.eth.wait_for_transaction_receipt(
                redeem_hash
            )
    ...
    async def cancel(self, overrides=None):
        ...
        if status == L1ToL2MessageStatus.FUNDS_DEPOSITED_ON_L2:
            ...
            tx_hash = await arb_retryable_tx.functions.cancel(
                self.retryable_creation_id
            ).transact(overrides)
...
class EthDepositMessage:
    ...
    async def status(self):
        receipt = await get_transaction_receipt(
            self.l2_provider, self.l2_deposit_tx_hash
        )
        if receipt is None:
            return "PENDING"
        else:
    ...
    async def wait(self, confirmations=None, timeout=None):
        ...
        chosen_timeout = (
            timeout if is_defined(timeout) else l2_network["depositTimeout"]
        )
