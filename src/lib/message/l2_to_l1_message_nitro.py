...
class L2ToL1MessageReaderNitro(L2ToL1MessageNitro):
    ...
    async def has_executed(self, l2_provider):
        l2_network = get_l2_network(l2_provider)  # Define get_l2_network based on your requirements
    ...
    async def get_block_from_node_log(self, l2_provider, log=None):
        ...
        if l2_block.send_root != parsed_log['afterState']['sendRoot']:
            raise Exception(f"L2 block send root doesn't match parsed log. {l2_block.send_root} {parsed_log['afterState']['sendRoot']}")
        return l2_block

    async def get_block_from_node_num(self, rollup_address, node_num, l2_provider):
        ...
        if await is_arbitrum_chain(self.l1_provider):
            try:
                l2_block_range = await get_block_ranges_for_l1_block_with_cache(self.l1_provider, l2_provider, created_at_block)
        ...
        event_fetcher = EventFetcher(rollup_contract.provider)
        logs = await event_fetcher.get_events(
            rollup_contract,
            lambda t: t.filters.NodeCreated(node_num),
            {
    ...
    async def get_first_executable_block(self, l2_provider):
        ...
        logs = await event_fetcher.get_events(
            rollup_contract,
            lambda t: t.filters.NodeCreated(),
            {
                'fromBlock': max(latest_block - l2_network.confirm_period_blocks - ASSERTION_CONFIRMED_PADDING, 0),
                'toBlock': 'latest',
        ...
        if last_send_count <= self.event['position']:
            return l2_network.confirm_period_blocks + ASSERTION_CREATED_PADDING + ASSERTION_CONFIRMED_PADDING + latest_block
