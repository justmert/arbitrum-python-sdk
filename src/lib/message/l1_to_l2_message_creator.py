...
class L1ToL2MessageCreator:
    ...
    async def get_ticket_creation_request(params, l1_provider, l2_provider, options=None):
        excess_fee_refund_address = params.get('excess_fee_refund_address', params.get('from'))
        call_value_refund_address = params.get('call_value_refund_address', params.get('from'))
        ...
        inbox_contract = load_contract('Inbox', l2_network.eth_bridge.inbox, l1_provider)

        function_data = inbox_contract.encodeABI(
            fn_name='createRetryableTicket',
            args=[
                params['to'], params['l2_call_value'], estimates['max_submission_cost'],
                excess_fee_refund_address, call_value_refund_address,
                estimates['gas_limit'], estimates['max_fee_per_gas'], params['data']
            ]
        )
        ...
        return {
            ...
            # isValid function needs to be implemented
        }
    ...
    async def create_retryable_ticket(self, params, l2_provider, options=None):
        ...
        create_request = await L1ToL2MessageCreator.get_ticket_creation_request(params, l1_provider, l2_provider, options)

        tx = await self.l1_signer.send_transaction({
            **create_request['tx_request'],
            **overrides
        })
